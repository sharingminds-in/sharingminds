...
const FILTER_MODEL_ID = process.env.CONTENT_FILTER_MODEL ?? 'meta-llama/llama-3.3-70b-instruct:free';
...
export function getAriaModel() {
  if (AI_PROVIDER === 'openrouter') {
    return createOpenRouterClient().chat(MODEL_IDS.openrouter);
  }
  return google(MODEL_IDS.google);
}

export function getFilterModel() {
  return createOpenRouterClient().chat(FILTER_MODEL_ID);
}
