...
import {
  ProfileValidators,
  useProfileState,
} from "@/components/shared/profile/base-profile";
...
import ProfileCompletionCard from "@/components/shared/profile/profile-track";
...
const MenteeProfileComponent = React.memo(() => {
  ...
  const handleInputChange = useCallback(
    (field: MenteeProfileFields, value: string) => {
  ...
  const profileCompletion = useMemo(() => {
    const fields = [
      state.data.currentRole,
      state.data.currentCompany,
      state.data.education,
      state.data.careerGoals,
      state.data.currentSkills,
      state.data.skillsToLearn,
  ...
  const missingFields = useMemo(() => {
    ...
    if (!state.data.currentRole?.trim()) missing.push("Current Role");

    if (!state.data.currentCompany?.trim()) missing.push("Company / School");

    if (!state.data.education?.trim()) missing.push("Education");

    if (!state.data.careerGoals?.trim()) missing.push("Career Goals");

    if (!state.data.currentSkills?.trim()) missing.push("Current Skills");

    if (!state.data.skillsToLearn?.trim()) missing.push("Skills To Learn");

    if (!state.data.interests?.trim()) missing.push("Interests");

    if (!state.data.learningStyle?.trim()) missing.push("Learning Style");

    if (!state.data.preferredMeetingFrequency?.trim())
      missing.push("Meeting Frequency");
        ...
        >
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm sm:text-base text-muted-foreground">
              Manage your mentee identity and preferences to find the best
              match.
            </p>
          </div>
                ...
                <div className="space-y-3">
                  ...
                  {state.isEditing ? (
                    <Textarea
                      ...
                      onChange={(e) =>
                        handleInputChange("currentSkills", e.target.value)
                      }
                          ...
                          {state.data.currentSkills
                            ...
                            .map((skill, index) => (
                              <Badge
                                key={index}
                                variant="secondary"
                                className="px-3 py-1 bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300"
                              >
                                {skill.trim()}
                              </Badge>
                            ))}
                ...
                <div className="space-y-3">
                  ...
                  {state.isEditing ? (
                    <Textarea
                      ...
                      onChange={(e) =>
                        handleInputChange("skillsToLearn", e.target.value)
                      }
                      ...
                      <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-md border border-slate-100 dark:border-slate-800 flex items-center">
                        {state.data.learningStyle ? (
                          <span className="capitalize">
                            {state.data.learningStyle.replace("-", " ")} Learner
                          </span>
                        ) : (
                          <span className="text-muted-foreground italic">
                            Not specified
                  ...
                  <div className="space-y-3">
                    ...
                    {state.isEditing ? (
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleInputChange("preferredMeetingFrequency", value)
                        }
                      >
                        <SelectTrigger>
                      ...
                      <div className="p-3 bg-slate-50 dark:bg-slate-900 rounded-md border border-slate-100 dark:border-slate-800 flex items-center">
                        {state.data.preferredMeetingFrequency ? (
                          <span className="capitalize">
                            {state.data.preferredMeetingFrequency.replace(
                              "-",
                              " ",
                            )}
                          </span>
